...
def build_error_diagram(failure):
    ...
    plt.bar(np.arange(failures), failure_count_values, color=get_colors(failures))
...
def build_dependency_diagram(name_column):
...
def find_value_error(column1, column2):
    array_param_first = []
    array_param_second = []
